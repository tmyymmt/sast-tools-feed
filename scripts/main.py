...
FEED_TITLE = "SAST Tools Feed"
FEED_BASE_URL = os.environ.get("FEED_BASE_URL", "https://tmyymmt.github.io/sast-tools-feed/feeds")
...
<body>
  <h1>SAST Tools Feed</h1>
  <p>Release information feeds for SAST (Static Application Security Testing) tools.</p>
  <p>Updated weekly via GitHub Actions. Subscribe with any RSS reader.</p>
...
def write_pages(tools: list, entries_by_tool: Dict[str, List[ReleaseEntry]]) -> None:
    ...
    write_file_atomic(
        ...
        render_html("SAST Tools Comparison", generate_comparison_page(tools, entries_by_tool)).encode("utf-8"),
    )
    write_file_atomic(
        PAGES_DIR / "comparison_ja.html",
        render_html("SASTツール比較", generate_comparison_page_ja(tools, entries_by_tool), lang="ja").encode("utf-8"),
    )
